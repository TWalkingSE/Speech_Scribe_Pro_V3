...
def cmd_transcribe(args, output_json: bool = False):
    ...
    from speech_scribe.core.exceptions import TranscriptionProcessError
    ...
    for file_path in args.files:
        ...
        try:
            ...
            if result:
                ...
                raise TranscriptionProcessError(str(path), "Resultado vazio")

        except Exception as e:
            if output_json:
                results.append({'file': str(path), 'error': str(e)})
            else:
                print_error(f"Erro: {path.name}: {e}")
...
def cmd_config(args, output_json: bool = False):
    ...
    from speech_scribe.core.config_manager import get_config_manager
...
def _format_srt(segments: List[dict]) -> str:
    ...
    from speech_scribe.utils.subtitle_formats import format_srt
    return format_srt(segments)


def _format_vtt(segments: List[dict]) -> str:
    """Formata segmentos em formato VTT"""
    from speech_scribe.utils.subtitle_formats import format_vtt
...
def cmd_batch(args, output_json: bool = False):
    ...
    if embed_subtitle:
        try:
            from speech_scribe.core.subtitle_embedder import check_ffmpeg, embed_subtitle_from_transcription
            from speech_scribe.core.subtitle_embedder import is_video_file as _is_video
            if not check_ffmpeg():
