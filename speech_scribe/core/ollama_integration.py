...
class OllamaManager:
    ...
    def _ensure_gpu_detected(self):
        """Lazily detect GPU info on first access to avoid importing torch during __init__."""
    ...
    def _send_generate_request(
        ...
        if response.status_code != 200:
            raise OllamaRequestError(f"Erro na API Ollama: {response.text}")
...
"""
    ...
    def analyze_transcription(self,
        ...
        if not model_name:
            ...
            if not model_name:
                raise OllamaError("Nenhum modelo Ollama disponível. Instale um modelo com 'ollama pull <modelo>'")
        ...
        try:
            ...
            raise OllamaRequestError(f"Timeout na análise com {model_name} (timeout={self.config.timeout}s)")
        except requests.exceptions.ConnectionError:
            raise OllamaNotAvailableError()
        except OllamaError:
            raise
        except Exception as e:
            raise OllamaError(f"Erro na análise: {e}")
    ...
    if manager.check_service_status():
        ...
        if best_model:
            ...
            try:
                ...
                print("✅ Teste de análise concluído com sucesso")
