...
try:
    from speech_scribe.core.history import get_history
...
try:
    from speech_scribe.core.exporters import DOCXExporter, JSONExporter, SRTExporter, TXTExporter, VTTExporter
...
try:
    from speech_scribe.core.translator import SUPPORTED_LANGUAGES, TranscriptionTranslator
...
try:
    from speech_scribe.core.subtitle_embedder import check_ffmpeg, embed_subtitle_from_transcription, is_video_file
    SUBTITLE_EMBEDDER_AVAILABLE = True
except ImportError:
    SUBTITLE_EMBEDDER_AVAILABLE = False

if PyQt6_available:
    from speech_scribe.gui.threads import AnalysisThread, TranscriptionThread
    from speech_scribe.gui.widgets import ModernUIBuilder
    ...
    try:
        from speech_scribe.gui.streaming import PYAUDIO_AVAILABLE, StreamingWidget
...
📦 Dependências:
   ...
   Ausentes: {', '.join(missing_deps) if missing_deps else 'Nenhuma'}
        ...
        def _display_analysis_results(self, results: Dict[str, Any]):
            ...
            try:
                from speech_scribe.gui.analysis_dialog import AnalysisResultsDialog
    ...
    class UpdateChecker(QThread):
        """Thread para verificação de atualizações em background"""
